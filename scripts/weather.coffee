...
get_data = (robot, msg, location, service, query, cb, lifetime, stack=0) ->
  ...
  if data?
    ...
    # get new data
    msg
      .http("http://api.weatherstack.com/current?access_key=#{process.env.HUBOT_APIXU_KEY}&units=f&query=#{encodeURIComponent query}")
      .get() (err, res, body) ->
...
send_temp = (msg, location, data) ->
  ...
  msg.send "#{report['weather_descriptions'].toString()} and #{report.temperature}°F (feels like #{report.feelslike}°F) with
#{report.wind_speed}mph wind in #{location.name}"
